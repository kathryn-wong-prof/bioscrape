...
kgrow = 33.333
kd = 2.2222
rxn1 = [[], ["X1"], "hillnegative", {"k":k, "s1":"X2","n":2, "K":K}]
...
M1.create_volume_rule("linear", {"growth_rate": g/10})
M1.create_volume_event("linear volume", {"growth_rate":g}, "hillpositive", {"k":kgrow, "s1":"X2", "n":2, "K":K})
M1.create_volume_event("linear volume", {"growth_rate":g/10}, "massaction", {"k":kgrow, "species":"X2"})
M1.create_death_event("death", {}, "hillpositive", {"k":kd, "s1":"X1", "n":4, "K":100*K})
M1.py_initialize()

rxn2 = [[], ["X2"], "hillnegative", {"k":k, "s1":"X1","n":2, "K":K}]
...
M2.create_volume_rule("linear", {"growth_rate": g/10})
#M2.create_volume_event("linear volume", {"growth_rate":g}, "hillpositive", {"k":kgrow, "s1":"X1", "n":2, "K":K})
M2.create_volume_event("linear volume", {"growth_rate":g/10}, "massaction", {"k":kgrow, "species":"X1"})
M2.create_death_event("death", {}, "hillpositive", {"k":kd, "s1":"X2", "n":4, "K":100*K})
M2.py_initialize()
...
while True:
	#cell_state_sample_list, sample_times = py_PropagateInteractingCells(timepoints, global_sync_period, sample_times = 3, model_list = model_list,initial_cell_states = initial_cell_counts, global_species = global_species, global_volume = global_volume, average_dist_threshold = average_dist_threshold)
	lineage_list, global_species, global_crn_result = py_SimulateInteractingCellLineage(timepoints, global_sync_period, model_list = model_list,initial_cell_states = initial_cell_counts, global_species = global_species, global_volume = global_volume, average_dist_threshold = average_dist_threshold)
...
if cell_state_sample_list is not None:
	...
	for i in range(len(sample_times)-1, -1, -1):
		for cell_type in range(2):
			...
			plt.hist(df["X1"], log = True, label = "Sample Time = "+str(i)+" Cells="+str(len(df["volume"])))
			if i == 0: plt.legend()

			plt.sca(axes[cell_type*3+2])
			plt.title("S histogram")
			plt.hist(df["X2"], log = True, label = "Sample Time = "+str(i)+" Cells="+str(len(df["volume"])))
			if i == 0: plt.legend()
